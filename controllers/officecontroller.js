...
exports.getavailableProperty = (req,res) =>{
  ...
  const query = `select p.address , r.price ,r.size,r.bedrooms from property p,r_house r where p.p_id = r.p_id and p.a_id ='r.a_id' and p.status like '%For%';
  select p.address , s.price ,s.size,s.bedrooms from property p,s_house s where p.p_id = s.p_id and p.a_id ='r.a_id' and p.status like '%For%';`
    connection.query(query,(err,rows,fields) => {
        if(!err){
...
exports.getPropertyRented = (req,res) =>{
    ...
    const query = ` select r.price,p.s_date,a.a_name,a.a_id,p.address,b.b_name,r.b_id from property p,r_house r,agent a,buyer b where
     p.p_id = r.p_id and r.a_id = a.a_id and r.b_id = b.b_id;`
    connection.query(query,(err,rows,fields) => {
        if(!err){
